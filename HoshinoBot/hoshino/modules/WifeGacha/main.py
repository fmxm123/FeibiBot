...
# 当超出次数时的提示
_add_max_notice = f'为防止滥用，管理员一天最多可添加{_add_max}次'
...
# 当超出次数时的提示
_ntr_max_notice = f'为防止牛头人泛滥，一天最多可牛{_ntr_max}次'
        ...
        async with session.begin():
            try:
                ...
                if random.random() < 0.1:
                    # 发生逃跑，清除老婆信息
                    await current_sv.remove_cid_by_user_group(ug)
                    await bot.send(ev, f"{ug_wife.name}不想被你日，逃跑了！")
                    return
